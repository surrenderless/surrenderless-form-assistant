...
export async function ensureMerchantContactFilingTask(
  ...
  if (!insertResult.ok) {
    console.warn("justice merchant contact task: insert", insertResult.error);
    return { task: null, timeline: null, created: false };
  }

  const task = insertResult.task;
  const companyContact = useCompanyContactLabels(intake);
  const appendedTimeline = insertResult.created
  ...
  return { task, timeline: appendedTimeline, created: insertResult.created };
}
