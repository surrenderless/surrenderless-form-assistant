...
export function buildIntakeChatAiMessages(
  ...
  const system = [
    ...
    "- company_name, company_website, company_contact_email, card_issuer_contact_email, purchase_or_signup, story",
    ...
    "- user_display_name, reply_email (reply_email = the USER's OWN contact email, i.e. the consumer's email — separate from company_contact_email and NEVER a company/merchant address)",
    ...
    "- company_contact_email (merchant/company support email for first outreach; empty string if unknown — do not invent)",
    "- When already_contacted is \"no\", Surrenderless sends the first message to the company itself, so proactively ask ONCE for the company's contact/support email and set company_contact_email if given. If the user doesn't have or know it, accept that, leave company_contact_email empty, and reassure them Surrenderless operators can still handle first outreach — never block on it.",
    "- card_issuer_contact_email (optional bank/card issuer dispute email for payment-dispute outreach; empty string if unknown — do not invent)",
    "",
    "problem_category enum:",
