...
export function ftcDesiredResolutionPhrase(category: JusticeIntake["problem_category"]): string {
  switch (category) {
...
/** Operator-oriented draft for Surrenderless-owned FTC consumer complaint filing. */
export function buildFtcComplaintDraft(intake: JusticeIntake): string {
  const issue = intake.problem_category.replace(/_/g, " ");
  const ask = ftcDesiredResolutionPhrase(intake.problem_category);
  const lines: string[] = [
    "DRAFT FOR FTC CONSUMER COMPLAINT (operator filing packet — paste into ReportFraud.ftc.gov / FTC consumer complaint)",
    "",
    `Company / business: ${intake.company_name}`,
    ...
    intake.consumer_us_state?.trim()
      ? `Consumer state: ${intake.consumer_us_state.trim().toUpperCase()}`
      : "",
  ];
