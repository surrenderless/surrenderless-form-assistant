...
/** Marker line that opens the owned-FCC delivery block inside task notes. */
export const FCC_OWNED_FILING_DELIVERY_BLOCK_MARKER = "---fcc_owned_autofill_delivery---";
const DELIVERY_BLOCK_MARKER = FCC_OWNED_FILING_DELIVERY_BLOCK_MARKER;

/** Persisted owned-FCC automation states (queued → submitting → filed | failed). */
export type FccOwnedFilingDeliveryState = "queued" | "submitting" | "failed" | "filed";

export type FccOwnedFilingDeliveryRecord = {
  delivery_state: FccOwnedFilingDeliveryState;
  provider: string;
  confirmation?: string;
  started_at?: string;
  completed_at?: string;
...
export function fccOwnedFilingIdempotencyKey(caseId: string): string {
  return `fcc-owned-autofill:${caseId.trim()}`;
}

export function fccOwnedFilingTimelineId(
  caseId: string,
  state: FccOwnedFilingDeliveryState
): string {
  return `fcc_autofill_${state}:${caseId.trim()}`;
}

export function parseFccOwnedFilingDeliveryRecord(
  notes: string | null | undefined
): FccOwnedFilingDeliveryRecord | null {
...
export function upsertFccOwnedFilingDeliveryNotes(
  notes: string | null | undefined,
  record: FccOwnedFilingDeliveryRecord
): string {
...
export function isFccOwnedFilingSubmitting(task: JusticeCaseTaskRow | undefined): boolean {
  if (!task || task.completed_at?.trim()) return false;
  return parseFccOwnedFilingDeliveryRecord(task.notes)?.delivery_state === "submitting";
}

export function isFccOwnedFilingFailed(task: JusticeCaseTaskRow | undefined): boolean {
  if (!task || task.completed_at?.trim()) return false;
  return parseFccOwnedFilingDeliveryRecord(task.notes)?.delivery_state === "failed";
}
