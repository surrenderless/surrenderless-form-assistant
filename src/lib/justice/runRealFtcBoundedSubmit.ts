...
async function applyFormDecision(page: Page, decision: FormDecision): Promise<void> {
  ...
  for (const field of fieldsToFill) {
    ...
    try {
      ...
      console.warn(`real-ftc-submit: could not fill "${field.selector}":`, message);
    }
  }
  ...
  if (decision.nextButton?.value?.trim()) {
    ...
    try {
      if (decision.waitForNavigation) {
        await Promise.all([
          page.waitForNavigation({ timeout: 10000 }).catch(() => {
            console.warn("real-ftc-submit: navigation timeout after button click");
          }),
      ...
      const message = err instanceof Error ? err.message : String(err);
      console.warn("real-ftc-submit: could not click button:", message);
    }
  }
}
