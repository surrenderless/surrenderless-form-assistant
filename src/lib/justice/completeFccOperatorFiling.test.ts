...
function commsIntake(): JusticeIntake {
 ...
 * pick CFPB after FCC completes — CFPB's priority (28) is below FCC's (27–70, always
 * >= 28 once cfpbRel is true), so it's excluded from the downstream ladder regardless
 * of evidence. */
function cfpbRelevantCommsIntake(): JusticeIntake {
  return buildJusticeIntakeFromParts({
    ...defaultBuildJusticeIntakeParts(),
...
function createFccCompleteSupabase(state: MockCaseState): SupabaseClient {
  return {
    from: (table: string) => {
...
describe("FCC workspace completion behavior", () => {
  beforeEach(() => {
  ...
  it("keeps workspace is_submitted false while requiring the same confirmation fields as the complete API", () => {
    const workspace = buildFccOperatorFilingWorkspace({ intake: commsIntake() });
  ...
  it("rejects completion without confirmation number (no false submitted state)", async () => {
    ...
    const state: MockCaseState = {
      ...
      task: {
        ...
        user_id: USER_ID,
        case_id: CASE_ID,
        title: "FCC filing: Metro Wireless",
        due_date: null,
        notes: `${marker}\ncase_id: ${CASE_ID}\ndraft:\nComplaint`,
        completed_at: null,
    ...
    const result = await completeFccOperatorFiling(createFccCompleteSupabase(state), USER_ID, {
      caseId: CASE_ID,
      taskId: TASK_ID,
      destination: "FCC",
      filedAt: "2026-04-15",
  ...
  it("records filing through the existing FCC completion path after portal confirmation", async () => {
    const marker = fccFilingTaskNotesMarker(CASE_ID);
    const workspace = buildFccOperatorFilingWorkspace({ intake: commsIntake() });
    const state: MockCaseState = {
      intake: commsIntake(),
    ...
    const result = await completeFccOperatorFiling(createFccCompleteSupabase(state), USER_ID, {
      ...
      filedAt: "2026-04-15",
      confirmationNumber: "FCC-998877",
      notes: "Filed via guided workspace",
    });

    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.filing.confirmation_number).toBe("FCC-998877");
    expect(result.filing.destination).toBe("FCC");
    expect(result.task.completed_at).toBeTruthy();
    expect(shouldQueueFccFilingTask(state.client_state)).toBe(false);
  });
  ...
  it("threads a hasUploadedEvidenceFile value derived from real evidence rows into the advance-after-completed call (shared boundary — independent of whether CFPB is reachable from this ladder position)", async () => {
    ...
    const buildState = (evidence: MockCaseState["evidence"]): MockCaseState => ({
      intake: cfpbRelevantCommsIntake(),
      client_state: {
        prepared_packet_approved: true,
        approved_next_action: {
          label: "FCC",
    ...
    await completeFccOperatorFiling(createFccCompleteSupabase(withFile), USER_ID, {
      caseId: CASE_ID,
      taskId: TASK_ID,
      destination: "FCC",
      filedAt: "2026-06-15",
    ...
    await completeFccOperatorFiling(createFccCompleteSupabase(withoutFile), USER_ID, {
      caseId: CASE_ID,
      taskId: TASK_ID,
      destination: "FCC",
      filedAt: "2026-06-15",
