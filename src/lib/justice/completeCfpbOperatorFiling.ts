...
export type CompleteCfpbOperatorFilingInput = {
  caseId: string;
  taskId: string;
  destination: string;
  filedAt: string;
...
export type CompleteCfpbOperatorFilingResult =
  | {
      ok: true;
...
export async function completeCfpbOperatorFiling(
  supabase: SupabaseClient,
  userId: string,
  input: CompleteCfpbOperatorFilingInput
): Promise<CompleteCfpbOperatorFilingResult> {
  ...
  if (destination !== canonicalDestination) {
    return { ok: false, error: "Invalid CFPB filing destination", status: 400 };
  }
  ...
  if (!taskNotesMatchCfpbFilingMarker(task.notes, caseId)) {
    return { ok: false, error: "Task is not a CFPB operator filing task", status: 400 };
  }
  ...
  if (filingsErr) {
    console.warn("justice cfpb operator filing: list filings", filingsErr.message);
    return { ok: false, error: filingsErr.message, status: 500 };
  }

  const cfpbFilings = cfpbFilingsForManualTracking((existingFilings ?? []) as JusticeCaseFilingRow[]);
  if (cfpbFilings.length > 0) {
    if (!hasCfpbFilingWithConfirmation(cfpbFilings)) {
      return {
        ok: false,
        error: "A CFPB filing record already exists for this case without confirmation",
        status: 409,
      };
    }
  }
  ...
  if (cfpbFilings.length > 0 && task.completed_at?.trim()) {
    idempotent = true;
    filing = cfpbFilings.find((f) => f.confirmation_number?.trim()) as JusticeCaseFilingRow;
  } else if (cfpbFilings.length > 0) {
    filing = cfpbFilings.find((f) => f.confirmation_number?.trim()) as JusticeCaseFilingRow;
    idempotent = true;
  } else {
    ...
    if (insertErr || !inserted) {
      console.warn("justice cfpb operator filing: insert", insertErr?.message ?? "failed");
  ...
  const taskResult = await completeCfpbFilingTaskIfOpen(supabase, userId, caseId, taskId);
  if (!taskResult.task) {
    return {
      ok: false,
      error: "Filing saved but could not complete the CFPB operator task",
      status: 500,
    };
  }
