...
describe("reconcileMissingOwnedFilingTasks", () => {
  beforeEach(() => {
    timelineStore.entries = [];
  });

  it("creates a missing demand-letter task for orphaned owned steps", async () => {
    const state: MockState = {
      cases: [
        ...
        },
        {
          ...
          user_id: USER_ID,
          intake: retailIntake(),
          client_state: { approved_next_action: { status: "completed" } },
          archived_at: null,
  ...
  it("does not duplicate when the owned filing task already exists", async () => {
  ...
  it("is idempotent across repeated reconciliation runs", async () => {
    ...
    const first = await reconcileMissingOwnedFilingTasks(createReconcileSupabase(state));
    expect(first.created).toBe(1);
    const second = await reconcileMissingOwnedFilingTasks(createReconcileSupabase(state));
