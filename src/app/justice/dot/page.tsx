...
export default function JusticeDotAviationPrepPage() {
  const { status: hydrationStatus, intake } = useJusticeActionPageHydration();
  const [copyHint, setCopyHint] = useState<string | null>(null);

  const complaintText = useMemo(() => (intake ? buildDotAviationComplaintDraft(intake) : ""), [intake]);
  ...
  const evidenceItems: string[] = [
    ...
    "Record locator / ticket number, flight numbers, and dates.",
    "Screenshots or exports of chats or emails with the airline or provider.",
    intake.already_contacted === "yes" && intake.contact_date
      ? `Documented contact (${intake.contact_method ?? "method"} on ${intake.contact_date}).`
      : "Proof you contacted the airline or company (dates, method, case or reference numbers).",
    intake.contact_proof_text?.trim() ? `Notes: ${intake.contact_proof_text.trim()}` : null,
    "Photos or receipts for baggage or service issues, if applicable.",
  ...
  return (
    <>
      ...
      <main className="mx-auto min-h-[calc(100vh-4rem)] max-w-lg bg-gradient-to-b from-neutral-50 to-neutral-100/80 px-4 py-8 pb-16 dark:from-neutral-950 dark:to-neutral-900 sm:px-6">
        ...
        <h1 className="mt-4 text-2xl font-bold text-neutral-900 dark:text-neutral-100">USDOT aviation complaint prep</h1>
        <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-400">
          Prepare text you can paste into the official U.S. Department of Transportation aviation consumer complaint
          process. This page does not submit anything for you.
        </p>
        ...
        <div className="mt-4 rounded-2xl border border-amber-200/90 bg-amber-50/95 p-4 text-sm text-amber-950 shadow-md shadow-amber-900/10 ring-1 ring-amber-950/[0.06] dark:border-amber-800 dark:bg-amber-950/45 dark:text-amber-100 dark:shadow-black/30 dark:ring-amber-500/10">
          <strong>Manual filing required.</strong> Complete any complaint yourself on the{" "}
          <a
            href={DOT_AIR_CONSUMER_URL}
            target="_blank"
            rel="noopener noreferrer"
            className="font-semibold text-amber-900 underline dark:text-amber-200"
            title="Opens the official DOT aviation consumer page in a new browser tab"
          >
            official DOT aviation consumer page
          </a>
          . Surrenderless does not file automatically — verify categories, company matching, and attachments on the
          official site.
        </div>
        ...
        <div className={`mt-6 ${cardCls}`}>
          ...
          <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-400">
            Flight / service / product: {intake.purchase_or_signup.trim() || "—"}
          </p>
          <p className="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
            Money: {intake.money_involved} · Travel or order / payment date: {intake.pay_or_order_date}
          </p>
        </div>

        <div className={`mt-5 ${cardCls}`}>
          <p className="text-xs font-semibold uppercase text-neutral-500 dark:text-neutral-400">
            Company / airline / provider
          </p>
        ...
        <div className={`mt-5 ${cardCls}`}>
          <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300">
            Copy-ready DOT aviation complaint text
          </label>
          <textarea
            readOnly
            ...
            rows={20}
            value={complaintText}
            aria-label="Draft DOT aviation complaint text"
          />
