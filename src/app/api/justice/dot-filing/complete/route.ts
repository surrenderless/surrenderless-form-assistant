...
import {
  completePlaywrightMockDotOperatorFiling,
...
export async function POST(req: NextRequest) {
  ...
  if (
    ...
    const mockResult = completePlaywrightMockDotOperatorFiling({
      caseId,
      userId: mockOwnerId,
      taskId,
      destination,
  ...
  const wrapped = await runWithBbbOwnedFilingSubmitContext(
    ...
    async () => {
      const result = await completeDotOperatorFiling(supabase, ownerResult.userId, {
        caseId,
        taskId,
        destination,
        filedAt,
