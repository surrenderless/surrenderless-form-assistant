import { NextResponse, type NextRequest } from "next/server";
import { validate as isUuid } from "uuid";
import { getUserOr401 } from "@/server/requireUser";
import { userOwnsJusticeCase } from "@/server/justiceCaseOwnership";
import { supabaseAdmin } from "@/utils/supabaseClient";

const MAX_TITLE = 500;
const MAX_DUE = 200;
const MAX_NOTES = 8000;

function nonEmptyString(v: unknown): v is string {
  return typeof v === "string" && v.trim().length > 0;
}

function optionalStringOrNull(v: unknown): string | null | undefined {
  if (v === undefined) return undefined;
  if (v === null) return null;
  if (typeof v !== "string") return undefined;
  const t = v.trim();
  return t.length === 0 ? null : t;
}

function clampLen(s: string, max: number): string {
  return s.length <= max ? s : s.slice(0, max);
}

export async function GET(req: NextRequest) {
  const userId = getUserOr401(req);
  if (!userId) {
    return NextResponse.json({ error: "Not signed in" }, { status: 401 });
  }

  const caseId = req.nextUrl.searchParams.get("case_id") ?? "";
  if (!isUuid(caseId)) {
    return NextResponse.json({ error: "Invalid case_id" }, { status: 400 });
  }

  if (!(await userOwnsJusticeCase(userId, caseId))) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }

  const { data, error } = await supabaseAdmin
    .from("justice_case_tasks")
    .select("id, user_id, case_id, title, due_date, notes, completed_at, created_at, updated_at")
    .eq("case_id", caseId)
    .eq("user_id", userId)
    .order("created_at", { ascending: false });

  if (error) {
    console.warn("justice_case_tasks list:", error.message);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }

  return NextResponse.json(data ?? []);
}

export async function POST(req: NextRequest) {
  const userId = getUserOr401(req);
  if (!userId) {
    return NextResponse.json({ error: "Not signed in" }, { status: 401 });
  }

  let body: unknown;
  try {
    body = await req.json();
  } catch {
    return NextResponse.json({ error: "Invalid JSON" }, { status: 400 });
  }

  if (body === null || typeof body !== "object" || Array.isArray(body)) {
    return NextResponse.json({ error: "Invalid body" }, { status: 400 });
  }

  const b = body as Record<string, unknown>;
  const caseId = typeof b.case_id === "string" ? b.case_id : "";
  if (!isUuid(caseId)) {
    return NextResponse.json({ error: "Invalid case_id" }, { status: 400 });
  }

  if (!(await userOwnsJusticeCase(userId, caseId))) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }

  if (!nonEmptyString(b.title)) {
    return NextResponse.json({ error: "title is required" }, { status: 400 });
  }
  const title = clampLen(b.title.trim(), MAX_TITLE);

  const dueDate = optionalStringOrNull(b.due_date);
  if (dueDate === undefined && b.due_date !== undefined && b.due_date !== null) {
    return NextResponse.json({ error: "Invalid due_date" }, { status: 400 });
  }
  const dueDateVal = dueDate == null ? dueDate : clampLen(dueDate, MAX_DUE);

  const notes = optionalStringOrNull(b.notes);
  if (notes === undefined && b.notes !== undefined && b.notes !== null) {
    return NextResponse.json({ error: "Invalid notes" }, { status: 400 });
  }
  const notesVal = notes == null ? notes : clampLen(notes, MAX_NOTES);

  const insertRow: Record<string, unknown> = {
    user_id: userId,
    case_id: caseId,
    title,
  };
  if (dueDateVal !== undefined) insertRow.due_date = dueDateVal;
  if (notesVal !== undefined) insertRow.notes = notesVal;

  const { data, error } = await supabaseAdmin
    .from("justice_case_tasks")
    .insert(insertRow)
    .select("id, user_id, case_id, title, due_date, notes, completed_at, created_at, updated_at")
    .single();

  if (error) {
    console.warn("justice_case_tasks insert:", error.message);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }

  return NextResponse.json(data);
}
