...
export async function POST(req: NextRequest) {
  ...
  if (
    ...
    const mockResult = completePlaywrightMockPaymentDisputeOperatorFiling({
      caseId,
      userId: mockOwnerId,
      taskId,
      destination,
  ...
  const wrapped = await runWithBbbOwnedFilingSubmitContext(
    ...
    async () => {
      const result = await completePaymentDisputeOperatorFiling(supabase, ownerResult.userId, {
        caseId,
        taskId,
        destination,
        filedAt,
