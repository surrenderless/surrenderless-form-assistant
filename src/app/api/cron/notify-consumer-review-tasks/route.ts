...
import { reconcileConsumerReviewTaskNotifications } from "@/lib/justice/reconcileConsumerReviewTaskNotifications";
...
async function handleCron(req: NextRequest): Promise<NextResponse> {
  ...
  const summary = await reconcileConsumerReviewTaskNotifications(supabase);
  // Surface partial failures to cron/uptime monitoring instead of always reporting success —
  // a task whose reply_email can never be resolved was previously silently re-failed forever
