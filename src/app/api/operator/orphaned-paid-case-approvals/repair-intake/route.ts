 ...
 * finalize endpoint.
 *
 * Everything that matters — the task-binding check, the compare-and-swap on updated_at, the
 ...
 * justice_cases.intake or .timeline can ever erase it. The RPC's idempotency key is derived from
 * the task plus the exact corrected content: an identical retry reuses the same key (a genuine
 * no-op), while a second, distinct correction for the same still-open task gets its own key and
 * its own audit event.
 */
