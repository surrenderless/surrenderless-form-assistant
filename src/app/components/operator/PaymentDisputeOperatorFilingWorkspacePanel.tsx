...
export function PaymentDisputeOperatorFilingWorkspacePanel({
  item,
  workspace,
  saving,
  onSubmit,
  ...
  workspace: PaymentDisputeOperatorFilingWorkspace;
  saving: boolean;
  onSubmit: (
    input: PaymentDisputeRecordInput
  ...
  return (
    <div className="mt-3 space-y-4">
      <p className="text-xs font-medium text-neutral-800 dark:text-neutral-200">
        Payment dispute guided filing workspace (operator fallback)
      </p>
      <p className="text-[11px] leading-relaxed text-neutral-600 dark:text-neutral-400">
        {workspace.delivery.operator_guidance}
      </p>
      <p className="text-[11px] font-medium text-amber-800 dark:text-amber-300">
        Not marked sent in-app from this panel. Dispute outreach is complete only after automated
        email acceptance or after you record a confirmation below. Status claimed here:{" "}
        ...
        <dl className="space-y-1.5 text-[11px] text-neutral-800 dark:text-neutral-100">
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <dd className="mt-0.5">{workspace.delivery.merchant_name}</dd>
            </div>
            <CopyButton value={workspace.delivery.merchant_name} label="Copy" />
          </div>
          <div className="flex items-start justify-between gap-2">
            <div>
              <dt className="text-[10px] font-medium uppercase tracking-wide text-neutral-500 dark:text-neutral-400">
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <dd className="mt-0.5">{workspace.charge_fields.charge_amount}</dd>
            </div>
            <CopyButton value={workspace.charge_fields.charge_amount} label="Copy" />
          </div>
          <div className="flex items-start justify-between gap-2">
            <div>
              <dt className="text-[10px] font-medium uppercase tracking-wide text-neutral-500 dark:text-neutral-400">
              ...
              <dd className="mt-0.5">{workspace.charge_fields.charge_date}</dd>
            </div>
            <CopyButton value={workspace.charge_fields.charge_date} label="Copy" />
          </div>
          <div className="flex items-start justify-between gap-2">
            <div>
              <dt className="text-[10px] font-medium uppercase tracking-wide text-neutral-500 dark:text-neutral-400">
              ...
              <dd className="mt-0.5">{workspace.charge_fields.payment_method}</dd>
            </div>
            <CopyButton value={workspace.charge_fields.payment_method} label="Copy" />
          </div>
          <div className="flex items-start justify-between gap-2">
            <div>
              <dt className="text-[10px] font-medium uppercase tracking-wide text-neutral-500 dark:text-neutral-400">
              ...
              <dd className="mt-0.5">{workspace.charge_fields.dispute_reason}</dd>
            </div>
            <CopyButton value={workspace.charge_fields.dispute_reason} label="Copy" />
          </div>
      ...
      <div className="space-y-2 rounded-lg border border-neutral-200/90 bg-neutral-50/80 p-3 dark:border-neutral-600 dark:bg-neutral-950/40">
        <div className="flex items-center justify-between gap-2">
          <p className="text-[11px] font-semibold text-neutral-800 dark:text-neutral-200">
            Prepared bank/card dispute letter
          </p>
          <CopyButton value={workspace.letter_draft} label="Copy full draft" />
        </div>
      ...
      >
        <p className="text-xs font-medium text-neutral-800 dark:text-neutral-200">
          Record payment dispute (after send confirmation)
        </p>
        ...
        <label className="block text-[11px] font-medium text-neutral-700 dark:text-neutral-300">
          ...
          <input
            ...
            placeholder="Issuer case / dispute reference"
            className="mt-1 w-full rounded-lg border border-neutral-300 bg-white px-2.5 py-1.5 text-xs text-neutral-800 dark:border-neutral-600 dark:bg-neutral-900 dark:text-neutral-100 disabled:opacity-60"
          />
        </label>
        <label className="block text-[11px] font-medium text-neutral-700 dark:text-neutral-300">
