...
function OrphanedPaidCaseApprovalInvalidIntakeForm({
  ...
  const caseVersion = item.orphaned_paid_case_approval_invalid_intake?.case_version ?? 0;
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setError(null);
    const result = await onSubmit({ intake: parsed, caseVersion });
    if (!result.ok) setError(result.error);
  }
      ...
      {items.map((item) => (
        ...
        >
          ...
          {(() => {
            ...
            if (panelKind === "demand_letter_workspace" && item.demand_letter_workspace) {
              return (
            ...
            if (panelKind === "payment_dispute_workspace" && item.payment_dispute_workspace) {
              return (
                <PaymentDisputeOperatorFilingWorkspacePanel
                  item={item}
                  workspace={item.payment_dispute_workspace}
                  saving={savingTaskId === item.task_id}
                  onSubmit={(input) => onRecordComplete(item, input)}
                />
              );
