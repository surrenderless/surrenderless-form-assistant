...
import type { DemandLetterOperatorFilingWorkspace } from "@/lib/justice/demandLetterOperatorFilingWorkspace";

type DemandLetterRecordInput = {
  destination: string;
  filedAt: string;
...
export function DemandLetterOperatorFilingWorkspacePanel({
  item,
  workspace,
  saving,
  onSubmit,
  ...
  workspace: DemandLetterOperatorFilingWorkspace;
  saving: boolean;
  onSubmit: (
    input: DemandLetterRecordInput
  ...
  return (
    <div className="mt-3 space-y-4">
      <p className="text-xs font-medium text-neutral-800 dark:text-neutral-200">
        Demand letter guided filing workspace
      </p>
      <p className="text-[11px] leading-relaxed text-neutral-600 dark:text-neutral-400">
        {workspace.delivery.operator_guidance}
      </p>
      <p className="text-[11px] font-medium text-amber-800 dark:text-amber-300">
        Not marked sent in-app from this panel. Delivery is complete only after automated email
        acceptance or after you record a send confirmation below. Status claimed here:{" "}
      ...
      >
        <p className="text-xs font-medium text-neutral-800 dark:text-neutral-200">
          Record demand letter send (after confirmation)
        </p>
        ...
        <label className="block text-[11px] font-medium text-neutral-700 dark:text-neutral-300">
          ...
          <input
            ...
            placeholder="Send confirmation or reference number"
            className="mt-1 w-full rounded-lg border border-neutral-300 bg-white px-2.5 py-1.5 text-xs text-neutral-800 dark:border-neutral-600 dark:bg-neutral-900 dark:text-neutral-100 disabled:opacity-60"
          />
        </label>
        <label className="block text-[11px] font-medium text-neutral-700 dark:text-neutral-300">
