...
import type { FtcOperatorFilingWorkspace } from "@/lib/justice/ftcOperatorFilingWorkspace";
...
export function FtcOperatorFilingWorkspacePanel({
  item,
  workspace,
  saving,
  onSubmit,
  ...
  workspace: FtcOperatorFilingWorkspace;
  saving: boolean;
  onSubmit: (input: FtcRecordInput) => Promise<{ ok: true } | { ok: false; error: string }>;
}) {
  ...
  return (
    <div className="mt-3 space-y-4">
      ...
      <div className="space-y-2 rounded-lg border border-neutral-200/90 bg-neutral-50/80 p-3 dark:border-neutral-600 dark:bg-neutral-950/40">
        ...
        >
          Open FTC ReportFraud complaint portal
        </a>
      </div>
      ...
      >
        <p className="text-xs font-medium text-neutral-800 dark:text-neutral-200">
          Record FTC filing (after portal confirmation)
        </p>
