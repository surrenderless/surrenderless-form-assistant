...
import type { CfpbOperatorFilingWorkspace } from "@/lib/justice/cfpbOperatorFilingWorkspace";

type CfpbRecordInput = {
  destination: string;
  filedAt: string;
...
export function CfpbOperatorFilingWorkspacePanel({
  item,
  workspace,
  saving,
  onSubmit,
  ...
  workspace: CfpbOperatorFilingWorkspace;
  saving: boolean;
  onSubmit: (input: CfpbRecordInput) => Promise<{ ok: true } | { ok: false; error: string }>;
}) {
  ...
  return (
    <div className="mt-3 space-y-4">
      ...
      >
        <p className="text-xs font-medium text-neutral-800 dark:text-neutral-200">
          Record CFPB filing (after portal confirmation)
        </p>
