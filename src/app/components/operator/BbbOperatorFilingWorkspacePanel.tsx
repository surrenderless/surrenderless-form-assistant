...
import type { BbbOperatorFilingWorkspace } from "@/lib/justice/bbbOperatorFilingWorkspace";
...
export function BbbOperatorFilingWorkspacePanel({
  item,
  workspace,
  saving,
  onSubmit,
  ...
  workspace: BbbOperatorFilingWorkspace;
  saving: boolean;
  onSubmit: (input: BbbRecordInput) => Promise<{ ok: true } | { ok: false; error: string }>;
}) {
  ...
  return (
    <div className="mt-3 space-y-4">
      <p className="text-xs font-medium text-neutral-800 dark:text-neutral-200">
        BBB guided filing workspace (operator fallback)
      </p>
      <p className="text-[11px] leading-relaxed text-neutral-600 dark:text-neutral-400">
        {workspace.portal.operator_guidance}
      </p>
      <p className="text-[11px] text-neutral-600 dark:text-neutral-400">
        Owned autofill enabled in this environment:{" "}
        {workspace.owned_autofill_enabled ? "yes" : "no"}. Use this panel only when the BBB queue
        item is still open.
      </p>
      <p className="text-[11px] font-medium text-amber-800 dark:text-amber-300">
        Not submitted in-app from this panel. Filing is complete only after successful owned autofill
        or after you record a portal confirmation below. Status claimed here:{" "}
      ...
      >
        <p className="text-xs font-medium text-neutral-800 dark:text-neutral-200">
          Record BBB filing (after portal confirmation)
        </p>
