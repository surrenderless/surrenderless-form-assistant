...
| **Submission draft preview** | `src/app/justice/preview/page.tsx` — user reviews what would be submitted **before** any filing. **Deterministic** plain text from **`buildSubmissionDraftPreview.ts`** remains **source of truth / fallback**. **Optional AI-assisted** draft: **`POST /api/justice/preview-draft`** (authenticated, rate-limited, **`OPENAI_API_KEY`** server-side only) + pure prompt helper **`buildSubmissionDraftAiPrompt.ts`**; bounded JSON body (intake, destination, optional evidence + timeline summary); optional **`case_id`** UUID gated with **`userOwnsJusticeCase`**; AI output **review-only**, nothing filed; UI discloses that case details go to **OpenAI**. **No** DB writes, schema changes, embedded webview, or filing automation — first **Consumer Justice** path that calls an LLM. |
