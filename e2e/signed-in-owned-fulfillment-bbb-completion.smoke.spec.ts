...
test("owned BBB queue → operator completes → resolution endgame stays in chat", async ({
  browser,
}) => {
  ...
  await seedActiveCaseBbbFilingStep(consumerPage);
  ...
  await bbbItem.locator('input[type="date"]').fill("2026-06-22");
  await bbbItem.getByPlaceholder("Portal confirmation or reference number").fill("e2e-ui-bbb-999");
  ...
  await expect(
    chatTranscript.getByText(buildChatCaseProgressNarrationMessage("bbb_confirmed"))
  ).toBeVisible({ timeout: 30_000 });
