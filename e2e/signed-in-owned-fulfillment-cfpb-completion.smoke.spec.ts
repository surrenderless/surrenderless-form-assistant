...
test("owned CFPB queue → operator completes → resolution endgame stays in chat", async ({
  browser,
}) => {
  ...
  await expect(
    chatTranscript.getByText(buildChatCaseProgressNarrationMessage("cfpb_queued"))
  ...
  const cfpbItem = operatorPage.locator("li").filter({ hasText: "Acme Retail" }).first();
  await expect(cfpbItem).toBeVisible({ timeout: 30_000 });
  await expect(cfpbItem.getByText("Step: CFPB filing")).toBeVisible();

  await cfpbItem.locator('input[type="date"]').fill("2026-06-22");
  await cfpbItem.getByPlaceholder("Portal confirmation or reference number").fill("e2e-ui-cfpb-999");
  const completeResponsePromise = operatorPage.waitForResponse(
    (res) =>
      res.request().method() === "POST" &&
      res.url().includes("/api/justice/cfpb-filing/complete"),
    { timeout: 30_000 }
  );
  await cfpbItem.getByRole("button", { name: "Mark fulfillment complete" }).click();
  ...
  await expect(operatorPage.locator("li").filter({ hasText: "CFPB filing" })).toHaveCount(0, {
    timeout: 30_000,
  });

  await expectUrlStaysOnChatAi(consumerPage);
  await expect(
    chatTranscript.getByText(buildChatCaseProgressNarrationMessage("cfpb_confirmed"))
  ).toBeVisible({ timeout: 30_000 });
