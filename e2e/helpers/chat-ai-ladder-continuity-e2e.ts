...
/**
 * BBB approved + packet approved so chat queues Surrenderless-owned BBB fulfillment.
 * Already-contacted no so completion is terminal for resolution endgame.
 */
export async function seedActiveCaseBbbFilingStep(page: Page): Promise<void> {
  const caseId = CHAT_AI_LADDER_CONTINUITY_E2E_CASE_ID;
  const intake = {
    ...buildPlaywrightMockE2eCaseIntake(),
    company_name: "Acme Retail",
...
/** Demand letter approved with company_contact_email so mock automated email delivery can run. */
export async function seedActiveCaseDemandLetterFilingStepWithCompanyEmail(
  page: Page
): Promise<void> {
  const caseId = CHAT_AI_LADDER_CONTINUITY_E2E_CASE_ID;
  const intake = {
  ...
  const approvedNextAction: JusticeApprovedNextAction = {
    ...
    status: "approved",
    approved_at: "2026-06-21T00:00:14.000Z",
  };
  await resetMockCase(page);
  await patchMockCase(page, {
    intake,
...
/** Demand letter approved with no company_contact_email — needs-recipient prompt state. */
export async function seedActiveCaseDemandLetterFilingStepNeedsRecipient(
  page: Page
): Promise<void> {
  const caseId = CHAT_AI_LADDER_CONTINUITY_E2E_CASE_ID;
  const intake = {
    ...buildPlaywrightMockE2eCaseIntake(),
    company_name: "Acme Retail",
    // No company_contact_email — the only diff vs. …WithCompanyEmail — is what keeps
    // showDemandLetterNeedsRecipientNotice true instead of a queued/sending state.
    problem_category: "online_purchase" as const,
    purchase_or_signup: "widget order",
    story: "Ordered a widget that never arrived and merchant refused a refund.",
    money_involved: "$89.00",
