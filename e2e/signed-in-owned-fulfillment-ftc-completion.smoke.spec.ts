...
test("owned FTC queue → operator completes → resolution endgame stays in chat", async ({
  browser,
}) => {
  ...
  await seedActiveCaseFtcFilingStep(consumerPage);
  ...
  await ftcItem.locator('input[type="date"]').fill("2026-06-22");
  await ftcItem.getByPlaceholder("Portal confirmation or reference number").fill("e2e-ui-ftc-999");
  ...
  await expect(
    chatTranscript.getByText(buildChatCaseProgressNarrationMessage("ftc_confirmed"))
  ).toBeVisible({ timeout: 30_000 });
