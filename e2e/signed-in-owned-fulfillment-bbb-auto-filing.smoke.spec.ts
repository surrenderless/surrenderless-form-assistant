...
import {
  ...
  seedActiveCaseBbbFilingStepWithOwnedAutofill,
...
test("owned BBB auto filing completes in chat without DIY controls", async ({ page }) => {
  test.setTimeout(180_000);

  await seedActiveCaseBbbFilingStepWithOwnedAutofill(page);
  ...
  await expect(page.getByText("BBB filing queued.")).toHaveCount(0);
});
